...
module LLVM
  ...
  {% for name, target in {
    ...
    def self.init_{{target.id}} : Nil
      return if @@initialized_{{target.id}}
      @@initialized_{{target.id}} = true

      \{% if LibLLVM::BUILT_TARGETS.includes?({{target.symbolize}}) %}
        LibLLVM.initialize_{{target.id}}_target_info
        LibLLVM.initialize_{{target.id}}_target
        LibLLVM.initialize_{{target.id}}_target_mc
        LibLLVM.initialize_{{target.id}}_asm_printer
        LibLLVM.initialize_{{target.id}}_asm_parser
        LibLLVM.link_in_mc_jit
      \{% else %}
        raise "ERROR: LLVM was built without {{name.id}} target"
      \{% end %}
    end
  {% end %}
