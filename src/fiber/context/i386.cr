...
class Fiber
  ...
  def self.swapcontext(current_context, new_context) : Nil
    ...
    {% if compare_versions(Crystal::LLVM_VERSION, "9.0.0") >= 0 %}
      ...
      movl $$0, 4(%ecx)   // current_context.status = :suspended

      movl $$1, 4(%eax)   // new_context.status = :running
    ...
    {% else %}
      ...
      movl $$0, 4($0)   // current_context.status = :suspended

      movl $$1, 4($1)   // new_context.status = :running
  ...
  def self.loadcontext(new_context) : Nil
    ...
    {% if compare_versions(Crystal::LLVM_VERSION, "9.0.0") >= 0 %}
      asm("
      movl $$1, 4(%ecx)   // new_context.status = :running
      movl 0(%ecx), %esp  // %esp = new_context.stack_top
      popl %esi           // pop callee-saved registers from the stack
      popl %ebp
      popl %ebx
      popl %edi           // pop first argument (for initial resume)
    ...
    {% else %}
      ...
      # instructions that breaks the context switching.
      asm("
      movl $$1, 4($0)   // new_context.status = :running
      movl 0($0), %esp  // %esp = new_context.stack_top
      popl %esi         // pop callee-saved registers from the stack
      popl %ebp
      popl %ebx
      popl %edi         // pop first argument (for initial resume)
