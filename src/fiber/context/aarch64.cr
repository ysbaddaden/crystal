...
class Fiber
  ...
  def self.swapcontext(current_context, new_context) : Nil
    ...
    {% if compare_versions(Crystal::LLVM_VERSION, "9.0.0") >= 0 %}
      ...
      str     x19, [x0, #0]
      mov     x19, #0               // current_context.status = :suspended
      str     x19, [x0, #8]

      mov     x19, #1               // new_context.status = :running
      str     x19, [x1, #8]
  ...
  def self.loadcontext(new_context) : Nil
    ...
    {% if compare_versions(Crystal::LLVM_VERSION, "9.0.0") >= 0 %}
      ...
      mov     x19, #1               // new_context.status = :running
      str     x19, [x0, #8]
      ldr     x19, [x0, #0]         // sp = new_context.stack_top (x19)
      mov     sp, x19
    ...
    {% else %}
      ...
      mov     x19, #1               // new_context.status = :running
      str     x19, [$0, #8]
      ldr     x19, [$0, #0]         // sp = new_context.stack_top (x19)
      mov     sp, x19
