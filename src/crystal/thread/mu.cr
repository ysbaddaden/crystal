...
class Thread
  ...
  struct MU
    ...
    # Instead of waking up waiters that must relock this mu, tries to transfer
    # waiters from *wake* into this mu's @waiters queue. Any waiter that is
    # still in *wake* after the transfer must be woken.
    protected def try_transfer(wake : Pointer(Crystal::Dll(Waiter)), first_waiter : Pointer(Waiter), all_readers : Bool) : Nil
      ...
      if (old_word & ANY_LOCK) != 0 &&
        ...
        old_word, success = @word.compare_and_set(old_word, old_word | SPINLOCK | WAITING, :acquire, :relaxed)
        ...
        # transfer the first waiter iff it can't acquire mu
        # (note: Waiter#waiting is already set)
        # (note: Waiter#remove_count has already been incremented)
        ...
        release_spinlock(old_word | set_on_release)
      end
    end
    ...
    private def transfer(waiter, from)
      ...
      # no need to set waiting (it's already true), no need to increment remove
      # count (it already has been) but we must tell CV#wait that the waiter has
      # been transferred and is no longer a CV waiter
