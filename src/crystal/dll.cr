...
module Crystal
  # :nodoc:
  #
  # Circular Doubly-Linked-List of T elements. An empty list is a NULL pointer,
  # a single element list is the element referencing itself.
  struct Dll(T)
    module Node
      macro included
        ...
        macro init(*args)
          ...
          ::Crystal::Dll(::{{@type}}).init(pointerof(\%node))
          pointerof(\%node)
        end
