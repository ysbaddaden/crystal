...
class Crystal::EventLoop::IOCP < Crystal::EventLoop
  ...
  def accept(socket : ::Socket) : {::Socket::Handle, Bool}?
    ...
    # buffer_size is set to zero to only accept the connection and don't receive
    # any data. That will be a different operation.
    #
    # > If dwReceiveDataLength is zero, accepting the connection will not result
    # in a receive operation. > Instead, AcceptEx completes as soon as a
    # connection arrives, without waiting for any data.
    #
    # TODO: Investigate benefits from receiving data here directly. It's hard to
    # integrate into the event loop and socket API.
    buffer_size = 0
