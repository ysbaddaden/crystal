...
# is protected by a mutual exclusion lock.
#
# Individual (de)allocations of objects are protected with a fine grained lock.
#
...
class Crystal::Evented::Arena(T, BLOCK_BYTESIZE)
  ...
  def get(index : Index) : Pointer(T)
    ...
    raise RuntimeError.new("invalid arena index: not allocated") unless entry.value.allocated?
    raise RuntimeError.new("invalid arena index: generation has changed") unless entry.value.generation == index.generation
    entry.value.pointer
  end

  # Returns a pointer to the object previously allocated at *index* or `nil` if
  # the object isn't allocated, the generation has changed, or *index* is out of
  # bounds.
  def get?(index : Index) : Pointer(T)?
  ...
  # nothing if the object isn't allocated, the generation has changed, or
  ...
  private def at(index : Int32, grow : Bool)
    ...
    if n >= @blocks.size
      raise IndexError.new unless grow
      @mutex.synchronize { unsafe_grow(n) if n >= @blocks.size }
    end

    entry = @blocks.to_unsafe[n] + j
  end

  private def at?(index : Int32)
  ...
  # Grows @blocks and allocates the new blocks.
