...
module Crystal::System::FileDescriptor
  ...
  private def system_lock(blocking : Bool, exclusive : Bool) : Nil
    ...
    if blocking && !system_blocking?
      return event_loop.lock_file(self, flags)
    end

    handle = windows_handle
    overlapped = LibC::OVERLAPPED.new
    ret = LibC.LockFileEx(handle, flags | LibC::LOCKFILE_FAIL_IMMEDIATELY, 0, 0xFFFF_FFFF, 0xFFFF_FFFF, pointerof(overlapped))
    error = WinError.value
    ...
    while true
      ...
      ret, error =
        {% if flag?(:execution_context) %}
          Fiber.syscall { {LibC.LockFileEx(handle, flags, 0, 0xFFFF_FFFF, 0xFFFF_FFFF, pointerof(overlapped)), WinError.value} }
        {% else %}
          sleep 100.milliseconds
          {LibC.LockFileEx(handle, flags | LibC::LOCKFILE_FAIL_IMMEDIATELY, 0, 0xFFFF_FFFF, 0xFFFF_FFFF, pointerof(overlapped)), WinError.value}
        {% end %}
    end
  end
  ...
  private def overlapped_lock_file(flags)
    IOCP::IOOverlappedOperation.run(windows_handle) do |operation|
      result = LibC.LockFileEx(windows_handle, flags, 0, 0xFFFF_FFFF, 0xFFFF_FFFF, operation)
  ...
  {% else %}
    ...
    private def self.reader_loop
      while true
        ...
        request.bytes_read = units_read.to_i32

        # we cross enqueue from a bare thread but Crystal::Scheduler.enqueue is
        # thread safe on win32 targets (so it's safe)
        Crystal::Scheduler.enqueue(request.fiber)
      end
    end
  {% end %}
end
